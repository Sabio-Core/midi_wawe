...
//открыть окно
	...
	//file.open("legend_of_zelda.mid", std::ios::in | std::ios::binary);
	...
	while (p < track[trk].size()) {
		...
		switch(event_type) {
			case 0xff: {
				...
				switch(ev) {
					...
					case 0x20: {
						...
						std::cout << " - MIDI Channel Prefix " << next_ch << std::endl;
					} break;
					case 0x2f:
					...
					case 0x54:{
						...
						std::cout << " - SMPTE Offset: "<< hr << "_" << mn << "_" << se << "_" << fr << "_" << ff << std::endl;
					} break;
					case 0x58: {
						...
						std::cout << " - Time Signature: Размер " << razm_v << "/"  << razm_n
					...
					case 0x7f:
						...
						break;
					default: {
						std::cout << "Неизвестное событие" << std::endl;
						return 1;
					}
				}
				break;
			...
			case 0x90 ... 0x9f: {
				...
				unsigned note = unsigned(track[trk][p++]) & 0x7fU;
				unsigned velocity = unsigned(track[trk][p++]) & 0x7fU;
				std::cout << " - Событие Note On на канале " << chan <<  std::hex << ", включаем ноту " << note << " со скоростью " << velocity << std::endl;
...
//	auto t2 = t1 + std::chrono::milliseconds(1000);
//
...
//		} while (t1 < t2);
//		std::cout << "1 секунда прошла" << std::endl;
//		std::cout.flush();
//		t2 = t2 + std::chrono::milliseconds(1000);
//	}
